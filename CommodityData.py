...
class CommodityData:
    def __init__(
        ...
        if (
            not isinstance(commodity_mapping, dict)
            or commodity not in commodity_mapping
        ):
            raise ValueError(
                "Commodity mapping must be a dictionary containing the given commodity"
            )
    ...
    def seasonal_dependence(self):
        ...
        decomposition = seasonal_decompose(
            df["Close"], model="multiplicative", period=365
        )
        decomposition.plot()
        plt.show()
