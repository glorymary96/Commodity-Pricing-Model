...
class CommodityModel:
    def __init__(self, dataset: Type[CommodityData], models: List[Tuple[str, object]]):
        ...
        for model_name, model in self.models:
            if not callable(getattr(model, "fit", None)) or not callable(
                ...
                raise ValueError(
                    f"The model {model_name} must have 'fit' and 'predict' methods."
                )
    ...
    def train_models(self):
        ...
        for model_name, model in self.models:
            ...
            self.plot_results(
                y_train, pred_train, df["Date"].iloc[:split_index], model_name
            )
            # Evaluate the model
    ...
    def predict(self, future_days: int):
        ...
        future_dates = pd.DataFrame(
            {"Days": [last_day + i for i in range(1, future_days + 1)]}
        )
