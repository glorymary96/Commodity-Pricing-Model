...
class CommodityStatsModel:
    ...
    def execute(self):
        ...
        try:
            ...
            model_exp_smoothing = ExponentialSmoothing(
                train["Close"], seasonal="add", seasonal_periods=7
            )
            fit_exp_smoothing = model_exp_smoothing.fit()
            forecast_exp_smoothing = fit_exp_smoothing.forecast(
            ...
            model_sarima = SARIMAX(
                train["Close"], order=(1, 1, 1), seasonal_order=(1, 1, 1, 7)
            )
    ...
    def plot_results(self, forecasts_exp_smoothing, forecasts_sarima):
        ...
        plt.plot(
            self.commodity_data["Date"],
            self.commodity_data["Close"],
            label="Historical Data",
            color="black",
        )
        # Plot Exponential Smoothing forecast
        forecast_dates = pd.date_range(
            self.commodity_data["Date"].iloc[-1],
            periods=self.forecast_steps + 1,
            freq="D",
        )[1:]
        plt.plot(
            forecast_dates,
            forecasts_exp_smoothing[self.commodity],
            color="blue",
            linestyle="--",
            marker="o",
        ...
        plt.plot(
            forecast_dates,
            forecasts_sarima[self.commodity],
            color="red",
            linestyle="-",
            marker="x",
