...
df_monthly["Date"] = pd.to_datetime(
    df_monthly["Date"].str.replace("M", "-"), format="%Y-%m"
)
...
for col in df_monthly.columns[1:]:
    try:
        ...
        es_model = ExponentialSmoothing(
            train[col], trend="add", seasonal="add", seasonal_periods=12
        ).fit()
        ...
        sarima_model = sm.tsa.SARIMAX(
            train[col], order=(1, 1, 1), seasonal_order=(1, 1, 1, 12)
        ).fit(disp=False)
        sarima_pred = sarima_model.forecast(steps=forecast_steps)
        sarima_metrics = evaluate_model(test[col], sarima_pred)

        # Store results
        results[col] = {"Exponential_Smoothing": es_metrics, "SARIMA": sarima_metrics}
        ...
        plt.plot(
            plot_data.index, plot_data[col], label="Historical Data", color="black"
        )
        plt.plot(
            test.index,
            es_pred,
